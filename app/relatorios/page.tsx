        ...
        {loading ? (
          ...
          <>
            ...
            <div className="g4 mb14">
              ...
              <div className="kpi">
                <div className="kpi-l">Taxa de Recebimento</div>
                <div className="kpi-v" style={{ color: "var(--ok)" }}>
                  {formatPercent(fatTotal > 0 ? recTotal / fatTotal * 100 : 0)}
                </div>
                <div className="kpi-s up">do faturado recebido</div>
                <div className="kpi-bar" style={{ width: `${fatTotal > 0 ? recTotal / fatTotal * 100 : 0}%`, background: "var(--ok)" }} />
              </div>
              <div className="kpi">
                <div className="kpi-l">Total Pedidos</div>
                <div className="kpi-v">{pedidos.length}</div>
                <div className="kpi-s">{pedidos.filter(p => ["Entregue","Finalizado"].includes(p.status)).length} finalizados</div>
                <div className="kpi-bar" style={{ width: "70%", background: "var(--acc2)" }} />
              </div>
              <div className="kpi">
                <div className="kpi-l">Ticket Médio</div>
                <div className="kpi-v">{formatBRL(fatTotal / (pedidos.length || 1))}</div>
                <div className="kpi-s">por pedido</div>
                <div className="kpi-bar" style={{ width: "60%", background: "var(--acc4)" }} />
              </div>
            </div>
            ...
            {tab === 0 && (
              <div className="g2">
                <div className="card">
                  ...
                  <div style={{ height: "120px", display: "flex", alignItems: "flex-end", gap: "6px", marginBottom: "8px" }}>
                    {meses.map((m, i) => (
                      <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
                        <div style={{
                          width: "100%",
                          height: `${m.faturado > 0 ? Math.max((m.faturado / maxFat) * 100, 4) : 4}px`,
                          borderRadius: "3px 3px 0 0",
                          background: m.faturado > 0 ? "var(--acc)" : "var(--surf3)",
                        }} />
                  ...
                  <div className="sr">
                    <div className="sl">Total Faturado</div>
                    <div className="sv" style={{ color: "var(--acc)" }}>{formatBRL(fatTotal)}</div>
                  </div>
                  <div className="sr">
                    <div className="sl">Total Recebido</div>
                    <div className="sv" style={{ color: "var(--ok)" }}>{formatBRL(recTotal)}</div>
                  </div>
                  <div className="sr">
                    <div className="sl">A Receber</div>
                    <div className="sv" style={{ color: "var(--warn)" }}>{formatBRL(fatTotal - recTotal)}</div>
                  </div>
                </div>
                      ...
                      {[...financeiro].sort((a, b) => Number(b.faturado) - Number(a.faturado)).map((f, i) => {
                        ...
                        return (
                          <tr key={f.cliente_id}>
                            ...
                            <td>
                              {risco === 0
                                ? <span className="chip cg">Zero</span>
                                : risco < 0.5
                                  ? <span className="chip cy">Médio</span>
                                  : <span className="chip cr">Alto</span>}
                            </td>
            ...
            {tab === 2 && (
              <div className="g2">
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        <div className="prg" style={{ width: "80px", height: "5px" }}>
                          <div className="prg-f" style={{
                            width: `${pedidos.length > 0 ? count / pedidos.length * 100 : 0}%`,
                            background: "var(--acc)",
                          }} />
                        </div>
                ...
                <div className="card">
                  ...
                  <div className="sr">
                    <div className="sl">Total de Pedidos</div>
                    <div className="sv">{pedidos.length}</div>
                  </div>
                  <div className="sr">
                    <div className="sl">Entregues / Finalizados</div>
                    <div className="sv" style={{ color: "var(--ok)" }}>
                      {pedidos.filter(p => ["Entregue","Finalizado"].includes(p.status)).length}
                    </div>
                  </div>
                  <div className="sr">
                    <div className="sl">Em Produção</div>
                    <div className="sv" style={{ color: "var(--acc4)" }}>
                      {pedidos.filter(p => p.status.includes("Produção")).length}
                    </div>
                  </div>
                  <div className="sr">
                    <div className="sl">Aguardando Otimização</div>
                    <div className="sv" style={{ color: "var(--warn)" }}>
                      {pedidos.filter(p => p.status === "Aguardando otimização").length}
                    </div>
                  </div>
                  <div className="sr">
                    <div className="sl">m² Total Processado</div>
                    <div className="sv" style={{ color: "var(--acc)" }}>
                      {pedidos.reduce((a, p) => a + Number(p.m2_total), 0).toFixed(2)} m²
                    </div>
                  </div>
                  <div className="sr">
                    <div className="sl">Valor Médio por Pedido</div>
                    <div className="sv">{formatBRL(fatTotal / (pedidos.length || 1))}</div>
                  </div>
                </div>
              </div>
            )}
