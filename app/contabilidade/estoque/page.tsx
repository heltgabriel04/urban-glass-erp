                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                  <div style={{ background: "var(--surf1)", border: "1px solid var(--b1)", borderRadius: "10px", padding: "18px 20px" }}>
                    ...
                    {cmv.vidro.custoIndisponivel.pedidos > 0 && (
                      <div style={{ fontSize: "11px", color: "var(--warn)", marginTop: "8px" }} title="A fila PEPS precisou de um lote com custo_m2 ainda não definido (ex.: importação pendente de decisão do contador) — excluído deste CMV, não tratado como custo zero">
                    ...
                    {cmv.vidro.dataEstimada.pedidos > 0 && (
                      <div style={{ fontSize: "11px", color: "var(--t3)", marginTop: "8px" }} title="Custo PEPS calculado usando pelo menos um lote com dt_entrada_estimada=true (migrado do saldo antigo, sem data real de nota fiscal) — ordem de consumo entre esses lotes não é garantida">
                        ℹ {cmv.vidro.dataEstimada.pedidos} pedido(s) neste CMV usaram lote(s) com data de entrada estimada — ordem PEPS não 100% confiável
                      </div>
                    )}
                  </div>
