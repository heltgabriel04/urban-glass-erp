...
function EtiquetaRetalhoCard({ r, num }: { r: Retalho; num: number }) {
  return (
      ...
      <div className="et-corpo">
        <div className="et-esq">
          <div className="et-linha">
            <span className="et-lbl">RETALHO</span>
            <span className="et-val et-cliente">{r.id}</span>
          </div>
          <div className="et-linha">
            <span className="et-lbl">MATERIAL</span>
            <span className="et-val">
              {r.produto_nome}
              {r.espessura ? ` · ${r.espessura}mm` : ""}
            </span>
          </div>
          <div className="et-linha et-dim">
            <span className="et-lbl">MEDIDAS</span>
            <span className="et-val et-medidas">L {r.largura} × H {r.altura} mm</span>
          </div>
        ...
        <div className="et-dir">
          <div className="et-m2">{formatM2(r.m2)}</div>
          <div className="et-qrlbl">ÁREA</div>
        </div>
      </div>
    </div>
  );
...
export default function EtiquetasRetalhosPage() {
  ...
  useEffect(() => {
    async function load() {
      const raw = sessionStorage.getItem("retalhos_etiquetas_ids");
      const ids: string[] = raw ? JSON.parse(raw) : [];
      if (ids.length === 0) { setLoading(false); return; }

      const { data, error } = await supabase.from("retalhos").select("*").in("id", ids);
      if (!error && data) {
        const porId = new Map((data as Retalho[]).map(r => [r.id, r]));
        setRetalhos(ids.map(id => porId.get(id)).filter((r): r is Retalho => !!r));
      }
      setLoading(false);
    }
    load();
  ...
  if (retalhos.length === 0)
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100vh", gap: "12px", fontFamily: "Arial" }}>
        <div style={{ color: "#c00", fontWeight: 700 }}>Nenhum retalho selecionado.</div>
        <button onClick={() => router.push("/retalhos")} style={{ padding: "8px 16px", borderRadius: "6px", border: "1px solid #ccc", cursor: "pointer" }}>
          ← Voltar
        </button>
      </div>
    );
  ...
  return (
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .et-dir {
          ...
          justify-content: center; gap: 3px; flex-shrink: 0; min-width: 80px;
        }
        .et-m2 {
          font-size: 22px; font-weight: 900; font-family: 'Courier New', monospace; color: #000;
        }
        .et-qrlbl {
      ...
      <div className="toolbar">
        <button className="btn-back" onClick={() => router.push("/retalhos")}>← Voltar</button>
        <div className="toolbar-title">
          Etiquetas de Retalhos
          <span style={{ fontSize: "11px", color: "#aaa", marginLeft: "12px" }}>{retalhos.length} etiqueta(s)</span>
        </div>
      ...
      <div className="grid-wrapper">
        {retalhos.map((r, i) => (
          <EtiquetaRetalhoCard key={r.id} r={r} num={i + 1} />
        ))}
      </div>
    </>
  );
