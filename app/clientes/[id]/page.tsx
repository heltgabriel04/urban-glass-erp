...
export default function ClienteDetalhe() {
  ...
  if (loading) {
    return <AppLayout><div className="con"><div className="loading">Carregando cliente...</div></div></AppLayout>;
  }

  if (!cliente) {
    return <AppLayout><div className="con"><div style={{ color: "var(--err)", padding: "32px" }}>Cliente não encontrado.</div></div></AppLayout>;
  }
    ...
    <AppLayout>
      ...
      <div className="tb">
        ...
        <div className="tb-title" style={{ flex: 1 }}>
          {cliente.nome}
          {!cliente.ativo && (
            <span className="chip cr" style={{ marginLeft: "10px", fontSize: "10px" }}>Inativo</span>
          )}
        </div>
        <button className="btn bg sm" onClick={() => router.push(`/clientes?edit=${cliente.id}`)}>
          Editar Cliente
        </button>
      </div>
      ...
      <div className="con" style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        <div className="g4">
          <div className="kpi">
            ...
            <div className="kpi-v" style={{ color: "var(--acc)" }}>{formatBRL(faturado)}</div>
            <div className="kpi-s">{pedidos.length} pedido(s)</div>
            <div className="kpi-bar" style={{ width: "100%", background: "var(--acc)" }} />
          </div>
          <div className="kpi">
            <div className="kpi-l">Recebido</div>
            <div className="kpi-v" style={{ color: "var(--ok)" }}>{formatBRL(recebido)}</div>
            <div className="kpi-s up">{formatPercent(pctRec)} do faturado</div>
            <div className="kpi-bar" style={{ width: `${pctRec}%`, background: "var(--ok)" }} />
          </div>
          <div className="kpi">
            <div className="kpi-l">A Receber</div>
            <div className="kpi-v" style={{ color: aReceber > 0 ? "var(--warn)" : "var(--t2)" }}>
              {formatBRL(aReceber)}
            </div>
            <div className={`kpi-s ${aReceber > 0 ? "wa" : ""}`}>
              {aReceber > 0 ? "⚠ Em aberto" : "✓ Quitado"}
            </div>
            <div className="kpi-bar" style={{ width: `${faturado > 0 ? aReceber / faturado * 100 : 0}%`, background: "var(--warn)" }} />
          </div>
          <div className="kpi">
            <div className="kpi-l">Ticket Médio</div>
            <div className="kpi-v">{formatBRL(ticketMed)}</div>
            <div className="kpi-s">{pedAtivos.length} em andamento</div>
          ...
          <div className="card" style={{ padding: "20px 24px" }}>
            ...
            <div style={{ marginTop: "20px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", color: "var(--t3)", marginBottom: "6px" }}>
                <span>Recebimento geral</span>
                <span>{pctRec.toFixed(0)}%</span>
              </div>
              <div style={{ height: "6px", borderRadius: "3px", background: "var(--surf3)", overflow: "hidden" }}>
                <div style={{
                  height: "100%", borderRadius: "3px",
                  width: `${pctRec}%`,
                  background: pctRec >= 100 ? "var(--ok)" : pctRec >= 50 ? "var(--acc)" : "var(--warn)",
                  transition: "width .3s",
                }} />
              </div>
            </div>
            ...
            <div style={{ marginTop: "16px", display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              {faturado === 0
                ? <span className="chip cgr">—</span>
                : aReceber === 0
                  ? <span className="chip cg">Zero</span>
                  : aReceber / faturado < 0.5
        ...
        <div className="card" style={{ padding: "20px 24px" }}>
          ...
          {pedidos.length === 0 ? (
            <div style={{ color: "var(--t3)", padding: "24px 0", textAlign: "center" }}>
          ...
          {pedidos.length > 0 && (
            <div className="totbar" style={{ marginTop: "12px" }}>
              <div className="ti">
                ...
                <div className="tv">{pedidos.length}</div>
              </div>
              <div className="ti">
                <div className="tl">Em Andamento</div>
                <div className="tv" style={{ color: "var(--acc2)" }}>{pedAtivos.length}</div>
              </div>
              <div className="ti">
                <div className="tl">Valor Total</div>
                <div className="tv" style={{ color: "var(--acc)" }}>{formatBRL(faturado)}</div>
              </div>
              <div className="ti">
                <div className="tl">A Receber</div>
                <div className="tv" style={{ color: aReceber > 0 ? "var(--warn)" : "var(--ok)" }}>
                  {formatBRL(aReceber)}
                </div>
              </div>
            </div>
