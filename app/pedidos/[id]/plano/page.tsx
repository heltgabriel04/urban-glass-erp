    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        html, body { margin: 0; padding: 0; background: #ccc; font-family: Arial, sans-serif; font-size: 11px; color: #000; }
        .chapa-page {
          width: 210mm;
          min-height: 297mm;
          margin: 16px auto;
      ...
      {/* TOOLBAR */}
      <div className="no-print" style={{ position:"sticky", top:0, zIndex:100, background:"#111", padding:"8px 16px", display:"flex", alignItems:"center", gap:"10px" }}>
      ...
      {chapas.map((chapa, ci) => {
        ...
        const thStyle: React.CSSProperties = { padding:"3px 6px", fontWeight:700, color:"white", fontSize:"9px", textAlign:"right" as const };
        const tdStyle: React.CSSProperties = { padding:"3px 6px", fontSize:"9px", fontWeight:700, color:"#000" };
        ...
        return (
          <div key={ci} className="chapa-page">
              ...
              <tbody>
                <tr>
                  <td style={{ verticalAlign:"top", width:"55%" }}>
                    <div style={{ fontSize:"15px", fontWeight:900, color:"#000" }}>urbanglass</div>
                    <div style={{ fontSize:"8px", color:"#000", textTransform:"uppercase", letterSpacing:"1px", fontWeight:700 }}>Urban Glass Comércio Ltda</div>
                    <div style={{ fontSize:"8px", color:"#333", marginTop:"1px" }}>Av. Vereador Raymundo Hargreaves, 1250 – Fontesville – JF/MG</div>
                  </td>
                  <td style={{ verticalAlign:"top", textAlign:"right" }}>
                    <div style={{ fontWeight:900, fontSize:"13px", color:"#000" }}>PLANO DE CORTE</div>
                    <div style={{ fontWeight:700, fontSize:"9px", color:"#000" }}>{agora.toLocaleTimeString("pt-BR")} &nbsp; {agora.toLocaleDateString("pt-BR")}</div>
                    <div style={{ fontWeight:700, fontSize:"9px", color:"#000" }}>Pedido: {pedido.id} &nbsp;·&nbsp; Chapa {ci + 1} de {chapas.length}</div>
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <div style={{ border:"1px solid #555", padding:"5px 8px", fontSize:"9px", background:"#f5f5f5" }}>
              <div style={{ fontWeight:700, color:"#000" }}>Projeto: {pedido.id} &nbsp;·&nbsp; Montagem {ci + 1} – Quantidade: 1</div>
              <div style={{ fontWeight:700, color:"#000" }}>Chapa utilizada: {chapa.W}.0 x {chapa.H}.0 mm</div>
              <div style={{ fontWeight:700, color:"#000" }}>Material: {chapa.prod}</div>
              <div style={{ fontWeight:700, color:"#000" }}>Cliente: {pedido.clientes?.nome ?? "—"}</div>
            </div>

            {/* TOTAIS */}
            <table style={{ width:"100%", border:"1px solid #555", fontSize:"9px" }}>
              <thead>
              ...
              <tbody>
                {[
                  { label:"Totais",   qtd: chapa.placed.length, m2: usedM2, pct: aprovPct, color:"#155724" },
                  { label:"Peças",    qtd: chapa.placed.length, m2: usedM2, pct: aprovPct, color:"#000" },
                  { label:"Retalhos", qtd: retalhos.length,     m2: retM2,  pct: (retM2 / chapaM2 * 100).toFixed(2), color:"#1e5c30" },
                  { label:"Sobras",   qtd: 0,                   m2: 0,      pct: perdaPct, color:"#721c24" },
                ].map((row, ri) => (
                  <tr key={ri} style={{ borderBottom:"1px solid #ccc", background: ri % 2 === 0 ? "#f9f9f9" : "white" }}>
                    <td style={{ ...tdStyle, textAlign:"left" }}>{row.label}</td>
                    <td style={{ ...tdStyle, textAlign:"right" }}>{row.qtd}</td>
                    <td style={{ ...tdStyle, textAlign:"right", fontFamily:"monospace" }}>{row.m2.toFixed(3)}</td>
                    <td style={{ ...tdStyle, textAlign:"right", color: row.color }}>{row.pct}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {pequenas.length > 0 && (
              <div>
                <div style={{ fontSize:"9px", fontWeight:700, color:"#000", marginBottom:"3px" }}>Legenda para as peças pequenas:</div>
                <table style={{ border:"1px solid #ccc", fontSize:"9px" }}>
                  <thead>
                    <tr style={{ background:"#ddd" }}>
                      <th style={{ padding:"2px 5px", textAlign:"left", fontWeight:700, color:"#000" }}>Peça</th>
                      <th style={{ padding:"2px 5px", textAlign:"right", fontWeight:700, color:"#000" }}>Larg</th>
                      <th style={{ padding:"2px 5px", textAlign:"right", fontWeight:700, color:"#000" }}>Alt</th>
                      <th style={{ padding:"2px 5px", textAlign:"left", fontWeight:700, color:"#000" }}>Descrição</th>
                    </tr>
                  </thead>
                  <tbody>
                    {pequenas.map((p, pi) => (
                      <tr key={pi} style={{ borderBottom:"1px solid #eee" }}>
                        <td style={{ padding:"2px 5px", fontWeight:700, color:"#000" }}>{String.fromCharCode(65 + pi)}</td>
                        <td style={{ padding:"2px 5px", textAlign:"right", fontFamily:"monospace", fontWeight:700, color:"#000" }}>{p.l}.0</td>
                        <td style={{ padding:"2px 5px", textAlign:"right", fontFamily:"monospace", fontWeight:700, color:"#000" }}>{p.a}.0</td>
                        <td style={{ padding:"2px 5px", fontWeight:700, color:"#000" }}>Ret {ci + 1} {p.l}.0 x {p.a}.0</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div style={{ display:"grid", gridTemplateColumns: retalhos.length > 0 ? "1fr 1fr" : "1fr", gap:"8px" }}>
              <div>
                <div style={{ fontSize:"9px", fontWeight:700, color:"#000", marginBottom:"3px" }}>Lista total das peças desta montagem:</div>
                <table style={{ width:"100%", border:"1px solid #555", fontSize:"9px" }}>
                  <thead>
                    <tr style={{ background:"#222" }}>
                  ...
                  <tbody>
                    {chapa.placed.map((p, pi) => (
                      <tr key={pi} style={{ borderBottom:"1px solid #ddd", background: pi % 2 === 0 ? "#f9f9f9" : "white" }}>
                        <td style={{ padding:"2px 5px", fontWeight:700, color:"#000" }}>{pi + 1}</td>
                        <td style={{ padding:"2px 5px", textAlign:"right", fontFamily:"monospace", fontWeight:700, color:"#000" }}>{p.l}.0 x</td>
                        <td style={{ padding:"2px 5px", textAlign:"right", fontFamily:"monospace", fontWeight:700, color:"#000" }}>{p.a}.0</td>
                        <td style={{ padding:"2px 5px", textAlign:"right", fontWeight:700, color:"#000" }}>1 —</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                    ...
                    <tbody>
                      {retalhos.map((f, fi) => (
                        <tr key={fi} style={{ borderBottom:"1px solid #ccc", background: fi % 2 === 0 ? "#f0f9f4" : "white" }}>
                          <td style={{ padding:"2px 5px", textAlign:"right", fontFamily:"monospace", fontWeight:700, color:"#000" }}>{f.l}.0</td>
                          <td style={{ padding:"2px 5px", textAlign:"right", fontFamily:"monospace", fontWeight:700, color:"#000" }}>{f.a}.0</td>
